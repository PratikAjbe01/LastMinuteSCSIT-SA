...
import { ArrowLeft, Upload, FileText, Check, X, AlertCircle, ShieldCheck } from "lucide-react"
...
const UploadDocumentPage = () => {
  ...
  const handleUpload = async () => {
    if (!user || !user.isAdmin || user.isAdmin !== 'admin' || user.role !== 'admin') {
      setUploadStatus("error");
            ...
            <ul className="space-y-2 text-gray-300 text-xs sm:text-sm">
              ...
              <li className="flex items-start space-x-2">
                ...
                <span>For papers with multiple pages, upload each page separately with the same field values, adding a suffix like "- Page 1", "- Page 2", etc., to the paper name; for single-page papers, upload without a suffix</span>
                 ...
                 <li className="flex items-start space-x-2">
                <span className="text-green-400 mt-1">•</span>
                <span>To upload common files for whole course ex- mca than choose semester value as "Whole Course{'(Your Course)'}", in ex case select semester value as "{'Whole Course(MCA)'}".</span>
              </li>
                 <li className="flex items-start space-x-2">
                <span className="text-green-400 mt-1">•</span>
                <span>To upload common files for whole semester ex- mca 3rd semester, than choose subject value as "Whole Semester", in ex case select subject value as "{'Whole Semester'}".</span>
              </li>
              <li className="flex items-start space-x-2">
                <span className="text-green-400 mt-1">•</span>
              ...
              <li className="flex items-start space-x-2">
                <span className="text-green-400 mt-1">•</span>
                <span>Select the correct course, semester, subject, file type, category, resourceType and a valid four-digit year (e.g., 2023) for proper categorization</span>
              </li>
              <li className="flex items-start space-x-2">
                <span className="text-green-400 mt-1">•</span>
